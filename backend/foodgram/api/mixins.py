...
class FavoriteShoppingCartMixin:
    def perform_action(self, user, recipe, model, response_text):
        if self.request.method == 'POST':
            ...
            serializer = RecipeSampleSerializer(recipe, context={'request': self.request})
    ...
    def favorite(self, request, **kwargs):
        ...
        return self.perform_action(user, recipe, Favorite, 'Такой рецепт уже в избранном!')

    @action(detail=True, methods=['post', 'delete'], permission_classes=[IsAuthenticated])
    def shopping_cart(self, request, **kwargs):
        user = self.request.user
        recipe = get_object_or_404(Recipe, id=kwargs['pk'])
        return self.perform_action(user, recipe, ShoppingCart, 'Такой рецепт уже есть!')
